...
array = input.split("\n")
array.each.with_index do |combined_string, i|
...
array.each do |small_array|
    small_array.each.with_index do |word, i|
        small_array[i] = word.chars.sort.join
    end
    unless small_array.length > small_array.uniq.length
        counter +=1
    end
end
