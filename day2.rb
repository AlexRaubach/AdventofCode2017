...
input.each do |string|    
    array << string.strip.split(" ").map! {|x| x.to_i}
end

@checksum = 0

def check_array(small_array)
    i=0
    found = false
    until found
        next_i = i + 1
        while next_i < small_array.length
            if small_array[i] < small_array[next_i]
                if small_array[next_i] % small_array[i] == 0
                    return small_array[next_i] / small_array[i]
                end
            else
                if small_array[i] % small_array[next_i] == 0
                    return  small_array[i] / small_array[next_i]
                end
            end
            next_i +=1
        end
